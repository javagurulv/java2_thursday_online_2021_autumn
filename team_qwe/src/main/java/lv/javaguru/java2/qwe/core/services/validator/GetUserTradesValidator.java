...
@Component
public class GetUserTradesValidator {

    private final Map<Predicate<GetUserTradesRequest>, CoreError> validator = ofEntries(
            entry(request -> request.getUserName().equals(""),
                    new CoreError("UserName", "must not be empty!"))
    );

    public List<CoreError> validate(GetUserTradesRequest request) {
