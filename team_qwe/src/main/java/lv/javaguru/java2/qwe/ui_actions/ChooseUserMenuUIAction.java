    ...
    public void execute() {
        ...
        while (userMenuOpen) {
            ...
            switch (type) {
                ...
                case "FIND USER BY NAME" -> new FindUserByNameUIAction(new FindUserByNameService(userData, new FindUserByNameValidator())).execute();
                case "GENERATE PORTFOLIO FOR USER" -> new GeneratePortfolioForUserUIAction(new GeneratePortfolioService(userData)).execute();
                case "SHOW USER PORTFOLIO" -> new ShowUserPortfolioUIAction(new ShowUserPortfolioService(userData, new ShowUserPortfolioValidator())).execute();
