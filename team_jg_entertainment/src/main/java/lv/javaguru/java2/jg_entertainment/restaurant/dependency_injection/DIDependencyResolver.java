...
public class DIDependencyResolver {
    ...
    public void resolve(ApplicationContext applicationContext, List<Class> diComponent){
        diComponent.forEach(diComp -> {
            Object diInstance = applicationContext.getBean(diComp);
            List<Field> diFields = findFieldsWithDIDependencyAnnotation(diComp);
